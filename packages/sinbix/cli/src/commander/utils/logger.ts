...
function createConsoleLogger(
  ...
  const logger = new logging.IndentLogger('clisinbix');

  if (!flags?.silent) {
    logger
      .pipe(filter((entry) => entry.level !== 'debug' || flags?.verbose))
      .subscribe((entry) => {
...
export const getLogger = (flags: LoggerFlags): logging.Logger => {
  ...
  return createConsoleLogger(flags, process.stdout, process.stderr, {
