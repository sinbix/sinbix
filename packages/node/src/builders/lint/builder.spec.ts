...
function createValidRunBuilderOptions(
  additionalOptions: Partial<LintBuilderSchema> = {}
): LintBuilderSchema {
  return {
    lintFilePatterns: [],
...
async function runBuilder(options: LintBuilderSchema) {
   ...
   */
  const { default: builderImplementation } = await import('./builder');
...
describe('Linter Builder', () => {
  ...
  it('should invoke the linter with the options that were passed to the builder', async () => {
    setupMocks();
    const { lint } = await import('./utils/eslint-utils');
    await runBuilder(
  ...
  it('should attempt to write the lint results to the output file, if specified', async () => {
    ...
    const { createDirectory } = await import('@sinbix/core');
    await runBuilder(
      createValidRunBuilderOptions({
        eslintConfig: './.eslintrc.json',
        lintFilePatterns: ['includedFile1'],
