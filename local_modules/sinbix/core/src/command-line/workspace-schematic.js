        ...
        workflow.reporter.subscribe((event) => {
            ...
            switch (event.kind) {
                case 'error':
                    ...
                    break;
                case 'update':
                    loggingQueue.push(core_1.tags.oneLine `${chalk_1.default.white('UPDATE')} ${eventPath} (${event.content.length} bytes)`);
                    break;
                case 'create':
                    loggingQueue.push(core_1.tags.oneLine `${chalk_1.default.green('CREATE')} ${eventPath} (${event.content.length} bytes)`);
                    break;
                case 'delete':
                    loggingQueue.push(core_1.tags.oneLine `${chalk_1.default.yellow('DELETE')} ${eventPath}`);
                    break;
                case 'rename':
                    const eventToPath = event.to.startsWith('/')
                        ? event.to.substr(1)
                        : event.to;
                    loggingQueue.push(core_1.tags.oneLine `${chalk_1.default.blue('RENAME')} ${eventPath} => ${eventToPath}`);
                    break;
            }
        });
