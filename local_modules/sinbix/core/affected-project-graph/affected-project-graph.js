...
const sinbix_json_changes_1 = require("./locators/sinbix-json-changes");
...
function filterAffected(graph, touchedFiles, workspaceJson = file_utils_1.readWorkspaceJson(), sinbixJson = file_utils_1.readSinbixJson(), packageJson = file_utils_1.readPackageJson()) {
    ...
    const touchedProjectLocators = [
        ...
        sinbix_json_changes_1.getTouchedProjectsInSinbixJson,
    ...
    const touchedProjects = touchedProjectLocators.reduce((acc, f) => {
        return acc.concat(f(touchedFiles, workspaceJson, normalizedSinbixJson, packageJson, graph));
    }, []);
