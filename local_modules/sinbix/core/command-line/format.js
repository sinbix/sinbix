...
function format(command, args) {
    const { sinbixArgs } = utils_1.splitArgsIntoSinbixArgsAndOverrides(args, 'affected');
    const patterns = getPatterns(Object.assign(Object.assign({}, args), sinbixArgs)).map((p) => `"${p}"`);
